...
def test_returns_at_most_top_n() -> None:
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels=LABELS, keywords=KEYWORDS, top_n=2))
    assert len(result) == 2


def test_returns_all_when_fewer_than_top_n() -> None:
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels=LABELS, keywords=KEYWORDS, top_n=10))
    assert len(result) == 3


def test_result_contains_expected_fields() -> None:
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels=LABELS, keywords=KEYWORDS, top_n=1))
    assert isinstance(result[0], TrendingTopic)


def test_total_likes_sum_correctly() -> None:
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels=LABELS, keywords=KEYWORDS, top_n=3))
...
def test_keywords_capped_at_five() -> None:
    ...
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels={1: LABELS[1]}, keywords=long_keywords, top_n=1))
    assert len(result[0].keywords) == 5


def test_topic_with_no_posts_excluded() -> None:
    labels_with_empty = {**LABELS, 99: {"label": "Empty"}}
    result = rank_topics(TopicRankingArgs(assignments=ASSIGNMENTS, posts_by_id=POSTS, labels=labels_with_empty, keywords=KEYWORDS, top_n=10))
